...
var NoumtexPurchase = function () {
    var max = 0;
    var LIST_LOADED = false;
    var LIST_CHANGED = false;
    var API_CONTROLLER = "NoumtexPurchase";
    ...
    return {
        init: function () {
                ...
                if (event.which == 13) {
                    if ($(this).val() == "") {
                        ...
                        if (PageSetting.BarCodeEnabled) {
                            ...
                            $("#item-container tbody tr:nth-last-child(1) input.Unit").focus().select();
                        }
                    }
            ...
            $(document).on("keyup", ".Rate,.ActualWeight", function () {
                var tr = $(this).closest("tr");
                $this.GetAmountTotal(tr);
            });
        ...
        AddSelectedChallans: function () {
            ...
            $("#DCTable > tbody tr").each(function () {
                ...
                if ($(tr).find("td:nth-child(1) input").is(":checked")) {
                    ...
                    dcnumbers += dcnumber + ",";
                    partycode = $(tr).find("td:nth-child(4)").text();
        ...
        GetAmountTotal: function (tr) {
            ...
            var type = $(tr).find("td:nth-child(10) .Type").val();
            if (type == "Weight") {
        ...
        SetFormControl: function () {
            if ($("#table-dc-detail tbody").children().length > 0) {
                ...
                $("#item-container tbody tr input").prop("disabled", true);
            }
        ...
        LoadPageSetting: function () {
            ...
            if (voucher.indexOf("gst") != -1) {
                $(".gst-row").removeClass("hide");
            }
            else {
                $(".gst-row").addClass("hide");
            }
