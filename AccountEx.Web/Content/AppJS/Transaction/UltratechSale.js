...
var UltratechSale = function () {
    var max = 0;
    var LIST_LOADED = false;
    var LIST_CHANGED = false;
    var API_CONTROLLER = "UltratechSale";
    ...
    return {
        ...
        SaveRecord: function (callback) {
            ...
            if (Common.Validate($("#mainform"))) {
                ...
                var gstamounttotal = $("#item-container tfoot tr").find(":nth-child(4) input.GSTAmount").val();
                var grossTotal = $("#item-container tfoot tr").find("input.Amount").val();
                record["GrossTotal"] = grossTotal;
                record["GstAmountTotal"] = gstamounttotal;
        ...
        GetQuantityPriceTotal: function (tr) {
            ...
            $(tr).find(":nth-child(6) input.Amount").val(amount.toFixed(2));

            var gstpercent = Common.GetInt($(tr).find(":nth-child(7) input.GSTPercent").val());
            var gstamount = Common.GetFloat(amount * gstpercent / 100);
            var netAmount = Common.GetInt(amount + gstamount);
            $(tr).find(":nth-child(8) input.GSTAmount ").val(gstamount.toFixed(2));
        ...
        GetWholeTotal: function () {
            ...
            $("#item-container tbody tr").each(function () {
                ...
                Price += Common.GetFloat($(this).find(":nth-child(6) input.Amount").val());
                GstAmount += Common.GetFloat($(this).find(":nth-child(8) input.GSTAmount").val());
        ...
        TemplatePrint: function (printType) {
            var Id = $("#Id").val();
