    ...
    return {
        init: function () {
            ...
            $("input[name='ac-type']").change(function () {
                ...
                if ($(this).val() == "cash") {
                    ...
                    if (!Common.isNullOrWhiteSpace(acc)) {
        ...
        MapComments: function () {
            ...
            if (type == "sale") {
                ...
                var html = "Book No:" + $("#InvoiceNumber").val() + ", Dc No: , Order No:";
                $("#Comments").val(html);
            }
        },
        New: function () {
        ...
        AddItem: function () {
            ...
            html += "<td><input type='text' class='DiscountAmount form-control input-small' disabled='disabled' readonly='readonly' /></td>";
            html += "<td><input type='text' class='NetAmount form-control input-small' disabled='disabled' readonly='readonly'></td>";
            html += "<td style=\"width: 8px;\"><span class=\"action\"><i class=\"fa fa-trash-o\" onclick=\"Pos.DeleteRow(this)\" data-original-title=\"Delete Item\"></i></span></td>";
            html += "</tr>";
        ...
        GetWholeTotal: function () {
            ...
            var rcvd = Common.GetFloat($("#Received").val());
            if (rcvd == 0) {
                rcvd = incAmount;
            }
            var balance = rcvd - incAmount
            $("#lblbalance").html(Common.GetInt(balance));
            $this.CalculatePreviousBalance();
            this.GetNetTotal();
        },
        GetNetTotal: function () {
            var total = Common.GetInt($("#NetTotal").val());
            ...
            var balance = rcvd - nettotal
            $("#lblbalance").html(Common.GetInt(balance));
        },
        CustomClear: function () {
            Common.Clear();
            $("#saleitem tbody").html("");
            $("#lblcurrentbalance,lblbalance").html("00");
