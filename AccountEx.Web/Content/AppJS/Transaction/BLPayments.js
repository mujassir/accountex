...
var BLPayments = function () {
    var max = 0;
    var LIST_LOADED = false;
    var LIST_CHANGED = false;
    var API_CONTROLLER = "BLPayment";
    ...
    return {
        init: function () {
            ...
            $(document).on("change", "select.BLNo", function (event) {

                var tr = $(this).closest("tr");
                if ($(this).val() != "") {
                    var bl = $.parseJSON($('option:selected', this).attr('data-detail'));;
                    $(tr).find("input.BLId").val(bl.BLId);
                    $(tr).find("input.BLChargeId").val(bl.Id);
                    $(tr).find("input.InvoiceNo").val(bl.InvoiceNo);
                    $(tr).find("input.Amount").val(bl.Amount);
                }
                else {
                    $(tr).find("input.BLId").val(0);
                    $(tr).find("input.BLChargeId").val(0);
                    $(tr).find("input.InvoiceNo").val("");
        ...
        LoadBL: function (tr) {
            ...
            var supplierId = Common.GetInt($(tr).find("input.AccountId").val());
            Common.WrapAjax({
                url: Setting.APIBaseUrl + API_CONTROLLER + "?key=GetPendingBLForPayment&supplierId=" + supplierId,
                type: "GET",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                blockUI: true,
                blockElement: "#form-info",
                blockMessage: "loading BLs..please wait",
                success: function (res) {
                    if (res.Success) {
                        var bls = res.Data;
                        ...
                        for (var i in bls) {
                            var bl = bls[i];
                            html += "<option data-detail='" + JSON.stringify(bl) + "'  value='" + bl.Id + "'>" + bl.BLNumber + "-" + bl.Charge + "</option>";
                        }
                        (tr).find("select.BLNo").html(html).select2();
        ...
        ListView: function () {
            var $this = this;          
            if (!LIST_LOADED) {
                var url = Setting.APIBaseUrl + API_CONTROLLER + "?loadDataTable=true" + "&type=" + VoucherType.blpayment;
                LIST_LOADED = true;
                DataTable.BindDatatable(DATATABLE_ID, url);
            }
        },
        ...
        LoadVoucher: function (key) {
            ...
            Common.WrapAjax({
                url: Setting.APIBaseUrl + API_CONTROLLER + "/" + voucherno + "?type=" + VoucherType[$this.GetType()] + "&key=loadvocuher&voucherkey=" + key + "&voucher=" + voucherno,
                type: "GET",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                blockUI: true,
                    ...
                    if (res.Success) {
                        ...
                        if (d == null) {
                            ...
                            if (d.Id > 0 && d.VoucherItems != null && d.VoucherItems.length > 0) {
                                ...
                                $("#btndelete,#btnsaverecord").prop("disabled", true);
