...
var CustomerIncentiveGroups = function () {
    var max = 0;
    var DATATABLE_ID = "mainTable";
    var API_CONTROLLER = "ItemGroup";
    ...
    return {
        init: function () {
                ...
                if (event.which == 13) {
                    if ($(this).val() == "") {
                        ...
                        if (PageSetting.BarCodeEnabled) {
                            var customer = Common.GetByBarCode($(this).val());
                            var tr = $(this).closest("tr");
                            if (typeof customer != "undefined" && customer != null) {
                                var account = Common.GetById(customer.AccountId);
                                $(this).val(account.AccountCode);
                                $(tr).find("input.CustomerId").val(customer.AccountId);
                                $(tr).find("input.Name").val(account.Name);
                                //$(tr).find("input.SaleLessRate").val(1);
            ...
            $(document).on("blur", ".Code", function () {
                if (!PageSetting.BarCodeEnabled) {
                    ...
                    var customer = Common.GetByCode($(this).val());
                    console.log(customer);
                    var tr = $(this).closest("tr");
                    if (typeof customer != "undefined" && customer != null) {
                        $(tr).find("input.CustomerId").val(customer.Id);
                        $(tr).find("input.Name").val(customer.Name);
                        var type = $this.GetType();
                            $this.AddItem();
            ...
            $(document).on("blur", ".Others", function (event) {

                var tr = $(this).closest("tr");
                var qty = Common.GetFloat($(tr).find("input.SaleLessRate").val());
                var code = $(tr).find("input.Code").val();

                if (qty <= 0) {
                    //var err = "Item " + code + " must have quantity greater than zero(0).";
                    //Common.ShowError(err);
                    $(tr).find(":nth-child(3) input.SaleLessRate").val("1");
            ...
            $(document).on("keyup", ".Others", function (event) {
                ...
                if (event.which == 13 && qty > 0)
                    ...
                    var err = "Item " + code + " must have SaleLessRate greater than zero(0).";
                    Common.ShowError(err);
                    $(tr).find("input.SaleLessRate").focus();
                }
            });
            $(document).on("keyup", ".SaleLessRate", function (event) {
                if (event.which == 13) {
                    $this.AddItem();
                }
            });
        ...
        GetQuantityPriceTotal: function (tr) {
            $this = this;
            var SaleLessRate = 0.0;
            var Rate = 0;
            SaleLessRate = Common.GetFloat($(tr).find(":nth-child(3) input.SaleLessRate").val());
            Rate = Common.GetInt($(tr).find(":nth-child(4) input.Rate").val());
            var amount = SaleLessRate * Rate;
        ...
        GetWholeTotal: function () {
            ...
            $("#item-container tbody tr").each(function () {
                SaleLessRate += Common.GetInt($(this).find(":nth-child(3) input.SaleLessRate").val());
            ...
            $("#item-container tfoot tr").find(":nth-child(2) input.SaleLessRate").val(SaleLessRate);
            ...
            $("#qtytotal1").val(SaleLessRate);
            $("#QuantityTotal").val(SaleLessRate);
            if (Price === 0)
                $("#GrossTotal").val("");
            else
        ...
        ListView: function () {
            ...
            var $this = this;
            if (!LIST_LOADED) {
                var url = Setting.APIBaseUrl + API_CONTROLLER + "?type=" + GroupSubType[$this.GetType()] + "&grouptype=" + GroupType["customerincentivegroup"];
                LIST_LOADED = true;
        ...
        Save: function () {
            ...
            if (Common.Validate($("#form-info"))) {
                ...
                if (Common.Validate($("#mainform"))) {
                    ...
                    Items = Enumerable.From(Items).Where("$.CustomerCode.trim()!=''").ToArray();
                    var err = "";
                    var type = $this.GetType();
                    for (var i in Items) {
                        var item = Items[i];
                        if (type.toLowerCase() == "less") {
                            //if (item.SaleLessRate <= 0) {
                            //    err += "Item " + item.ItemCode + "(" + item.ItemName + ") must have saleLessrate greater than zero(0).";
                            //}
                        }
                        var customer = Common.GetByCode(item.CustomerCode);
                        if (typeof customer == "undefined" || customer == null) {
                            err += +item.CustomerCode + " is not valid code.";
                        }
                    }
                    if (Items.length <= 0) {
                    ...
                    record["GroupSubType"] = GroupSubType[$this.GetType()];
                    record["GroupType"] = GroupType["customerincentivegroup"];
                    record["ItemGroupItems"] = Items;
                    Common.WrapAjax({
                        ...
                        blockMessage: "Saving CustomerIncentiveGroup...please wait",
                        success: function (res) {
                            if (res.Success) {
                                $this.ListView();
        ...
        Edit: function (id) {
            ...
            Common.WrapAjax({
                ...
                success: function (res) {
                    if (res.Success) {
                        var j = res.Data;
                        var items=j.ItemGroupItems;
                        for (var i in items)
                        {
                            var itemgroupitem = items[i];
                            var customer=Common.GetById(itemgroupitem.CustomerId);
                            items[i].CustomerCode = customer.AccountCode;
                            items[i].CustomerName = customer.Name;
                        }
                        $("#item-container tbody").html("");
        ...
        Delete: function (id) {
            ...
            Common.ConfirmDelete(function () {
                Common.WrapAjax({
                    ...
                    blockUI: true,
                    blockElement: "#div-table",
                    blockMessage: "Deleting CustomerIncentiveGroup...please wait",
                    success: function (res) {
                        if (res.Success) {
                            DataTable.RefreshDatatable(DATATABLE_ID);
                        }
            ...
            $(".Code").autocomplete({
                ...
                select: function (event, ui) {
                    ...
                    if (typeof account != "undefined" && account != null) {
                        $(tr).find(":nth-child(1) input.CustomerId").val(account.Id);
