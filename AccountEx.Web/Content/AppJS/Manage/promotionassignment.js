...
var promotionassignment = function () {
    var max = 0;
    var DATATABLE_ID = "mainTable";
    var API_CONTROLLER = "PromotionAssignment";
    ...
    return {
        init: function () {
            ...
            $(document).on("change", "#CustomerGroupId", function () {
                ...
                $("#Group").val(groupid);
                $this.GetPromotionWithCustomerGroup(groupid);
            });
            $(document).on("change", "#PromotionId", function () {
                var promotionid = $(this).val();
                $this.GetPromotionItems(promotionid);
            });
        ...
        GetPromotionItems: function (id) {
            var $this = this;
            var products = $("option:selected", $("#PromotionId")).attr("data-items");
            $("#Promotion").val($("#PromotionId").val());
            if (products != undefined && products != null && products != "")
                PRODUCTS = JSON.parse(products);
            $this.DrawTable();

        },
        GetPromotionWithCustomerGroup: function () {
            var $this = this;
            var groupId = $("#CustomerGroupId").val();

            Common.WrapAjax({
                url: Setting.APIBaseUrl + "Promotion/?groupId=" + groupId + "&key=loadGroupPromotion",
                type: "GET",
                ...
                success: function (res) {
                    if (res.Success) {
                        var promotions = res.Data.Promotions;
                        var html = "";
                        html += "<option></option>";
                        for (var i in promotions) {
                            var promotion = promotions[i];
                            html += "<option data-items='" + JSON.stringify(promotion.PromotionItems) + "' value='" + promotion.Id + "'>" + promotion.Name + "</option>";
                        }
                        $("#PromotionId").html(html).select2(
                                                     {
                                                         placeholder: Common.GetSelect2PlaceHolder($(this)),
                                                         minimumResultsForSearch: Setting.MinimumResultsForSearch
                                                     });
                        CUSTOMERS = res.Data.Group;
                        $this.DrawTable();
        ...
        Edit: function (groupId, promotionId) {
            var $this = this;
            $("#Promotion").val(promotionId);
            $("#Group").val(groupId);
            Common.UpdateRequired("#CustomerGroupId", false);
            Common.UpdateRequired("#PromotionId", false);
            $("#CustomerGroupId").addClass("hide");
            $("#PromotionId").addClass("hide");
            $("#PromotionLabel").removeClass("hide");
            $("#CustomerGroupLabel").removeClass("hide");
            Common.WrapAjax({
                url: Setting.APIBaseUrl + API_CONTROLLER + "?groupId=" + groupId + "&promotionId=" + promotionId + "&key=edit",
                type: "GET",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                blockUI: true,
                ...
                success: function (res) {
                    if (res.Success) {
                        var promotions = res.Data.Promotions;
                        $("#PromotionLabel label").text(promotions.Name);
        ...
        DrawTable: function () {
            ...
            var promotionItems = PRODUCTS;
            var customers = CUSTOMERS.ItemGroupItems
            if (promotionItems == undefined || promotionItems.length == 0 || customers == undefined) {
            ...
            for (var i in customers) {
                ...
                for (var j in promotionItems) {
                    var item = promotionItems[j];
                    html += "<td>";
                    html += "<input type='hidden' class='Id' value='0'/><input type='hidden' class='ItemId' value='" + item.ItemId + "'/><input type='text' class='SaleRate' value='" + item.PromotionRateSale + "' />";
                    html += "</td>";
                }
                html += "</tr>";
            }
        ...
        DrawEditTable: function (customerPromotions) {


            var promotionItems = Enumerable.From(customerPromotions.PromotionItems).Select("$.ItemId").Distinct().ToArray();
            var customers = Enumerable.From(customerPromotions.PromotionItems).Select("$.CustomerId").Distinct().ToArray();
            //if (promotionItems == undefined || promotionItems.length == 0 || customers == undefined) {
            //    $("#CustomerItemTable").addClass("hide");
            //    return;
            //}
            ...
            for (var i in customers) {
                ...
                for (var j in promotionItems) {
                    var item = promotionItems[j];
                    var account = Common.GetById(item);
                    var customerPromotion = Enumerable.From(customerPromotions.PromotionItems).FirstOrDefault(null, function (p) { return p.ItemId == item && p.CustomerId == customer });
                    html += "<td>";
                    html += "<input type='hidden' class='Id' value='" + customerPromotion.Id + "'/><input type='hidden' class='ItemId' value='" + item + "'/><input type='text' class='SaleRate' value='" + customerPromotion.PromotionRateSale + "' />";
                    html += "</td>";
                }
                html += "</tr>";
            }
        ...
        ListView: function () {
            ...
            Common.UpdateRequired("#PromotionId", true);
            $("#CustomerGroupId").removeClass("hide");
            $("#PromotionId").removeClass("hide");
            $("#PromotionLabel").addClass("hide");
                    ...
                    $("#CustomerItemTable tbody tr").each(function () {
                        ...
                        $(this).find("td:not(:first-child)").each(function () {
                            Items.push(
                           {
                               ...
                               PromotionRateSale: $(this).find('input.SaleRate').val(),
        ...
        Delete: function (groupId, promotionId) {
            var $this = this;
            Common.ConfirmDelete(function () {
            Common.WrapAjax({
                url: Setting.APIBaseUrl + API_CONTROLLER + "?groupId=" + groupId + "&promotionId=" + promotionId + "&key=delete",
                type: "DELETE",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                blockUI: true,
