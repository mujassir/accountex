...
var DetailedGeneralLedger = function () {
    ...
    return {
                    ...
                    if (res.Success) {
                        ...
                        for (var i in data) {
                            ...
                            if ($.inArray(data[i].TransactionType, voucherTypes) !== -1) {
                                html += "<td>" + $this.GetVoucherDetail(data[i].VoucherNumber, data[i].TransactionType, res.Data.SalesDetail) + "</td>";
                            } else {
        ...
        GetVoucherDetail: function (voucherNo, transactionType, sales) {
            var records = Enumerable.From(sales).Where(function (p) { return p.VoucherNumber == voucherNo && p.TransactionType == transactionType }).ToArray();
