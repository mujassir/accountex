...
{
    public class LaboursController : BaseApiController
    {
        ...
        public ApiResponse Get(int id)
        {
            ...
            try
            {
                ...
                if (key == "nextvouchernumber")
                    voucherNumber = transactionRepo.GetNextVoucherNumber(VoucherType.Labours);
                var data = workInProgressRepo.GetByVoucherNumberLabours(voucherNumber, key, out next, out previous);
                if (data != null)
                {
        ...
        public ApiResponse Post([FromBody]WorkInProgress input)
        {
            ...
            try
            {
                ...
                if (err == "")
                {
                    ...
                    LaboursManager.Save(input, true, false);
        ...
        {
            var coloumns = new[] { "VoucherNumber", "InvoiceNumber", "Date", "TotalQty", "TotalAmount", "" };
            var queryString = Request.RequestUri.ParseQueryString();
            ...
            foreach (var item in orderedList.Skip(displayStart).Take(displayLength))
            {
                ...
                data.Add(Numerics.IntToString(item.QuantityTotal));
                data.Add(Numerics.IntToString(item.NetTotal));
                var editIcon = "<i class='fa fa-edit' onclick=\"Labour.Edit(" + item.VoucherNumber + ")\" title='Edit' ></i>";
        ...
        public ApiResponse Delete(int id)
        {
            ...
            try
            {
                ...
                if (err == "")
                {
                    ProductionManager.Delete(voucherNo, VoucherType.Labours);
