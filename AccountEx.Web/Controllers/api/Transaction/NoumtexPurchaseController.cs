...
{
    public class NoumtexPurchaseController : BaseApiController
    {
        ...
        public virtual ApiResponse Get(string key, string dcIds)
        {
            ...
            try
            {
                ...
                var data = new DeliveryChallanRepository().AsQueryable(true).Where(p => Ids.Contains(p.Id)).
                    SelectMany(p => p.DCItems).GroupBy(q => new { q.ItemId, q.ItemCode, q.ItemName, q.Weight, q.MainUnitQuantity, q.Rate }).Select(r => new
                {
                    ...
                    r.Key.ItemName,
                    r.Key.Weight,
                    r.Key.MainUnitQuantity,
                    r.Key.Rate,
                    GSTPercent = r.FirstOrDefault().GSTPercent,
                    GSTAmount = r.Sum(m => m.GSTAmount),
        ...
        public ApiResponse Post([FromBody]Sale input)
        {
            ...
            try
            {
                ...
                if (err == "")
                {
                    ...
                    bool addtransaction = (input.TransactionType  == VoucherType.PurchaseReturn || input.TransactionType  == VoucherType.GstPurchaseReturn) ? true : false;
