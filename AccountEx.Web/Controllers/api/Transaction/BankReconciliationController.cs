...
using AccountEx.Web.Controllers.api.Transaction;

namespace AccountEx.Web.Controllers.api.Manage
{
    public class BankReconciliationController : GenericApiController<Voucher>
    {
        protected override JQueryResponse GetDataTable()
        {
            var queryString = Request.RequestUri.ParseQueryString();
            var coloumns = new[] { "AccountName","Date", "" };
            ...
            foreach (var item in orderedList.Skip(displayStart).Take(displayLength))
            {
                ...
                data.Add(item.Date.ToShortDateString());
                
               

                var editIcon = "<i class='fa fa-edit' onclick=\"Cities.Edit(" + item.Id + ")\" title='Edit' ></i>";
                var deleteIcon = "<i class='fa fa-trash-o' onclick=\"Cities.Delete(" + item.Id + ")\" title='Delete' ></i>";
