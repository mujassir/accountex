...
{
    public class InternalStockTransferReceiveController : BaseApiController
    {
        ...
        public ApiResponse Post([FromBody] InternalStockTransfer input)
        {
            ApiResponse response;
            try
            {
                ...
                if (err == "")
                {
                    ...
                    InternalStockTransferManager.Save(input, true);
        ...
        public ApiResponse Delete(int id)
        {
            ...
            try
            {
                ...
                if (err == "")
                {
                    new InternalStockTransferRepository().DeleteByVoucherNumber(id);
        ...
        private string ServerValidateSave(InternalStockTransfer input)
        {
            var err = ",";
            try
            {
                ...
                var Itemcountlist = input.InternalStockTransferItems.GroupBy(p => p.ItemId).Select(p => new
                {
                    ItemId = p.Key,
                ...
                if (input.Id > 0)
                {
                    var dbinternalStockTransfer = repo.GetById(input.Id);
                    if (dbinternalStockTransfer.VoucherNumber != input.VoucherNumber)
                    {
                        err += "can't change SRN no.please use previous SRN no.(" + dbinternalStockTransfer.VoucherNumber + "),";
                    }
                }
            }
            catch (Exception)
        ...
        private string ServerValidateDelete(int id, int locationId)
        {
            ...
            try
            {
                ...
                var internalStockTransfer = repo.GetByVoucherNo(id);

                if (internalStockTransfer != null)
                {
                    var order = orderbookingrepo.GetBySRN(internalStockTransfer.VoucherNumber, VoucherType.PurchaseOrder, internalStockTransfer.FromLocationId);
                    if (order != null)
                    {
        ...
        protected JQueryResponse GetDataTable()
        {
            ...
            var records = new InternalStockTransferRepository().AsQueryable(true).Where(p => p.Status == (byte)TransactionStatus.Pending);
            ...
            foreach (var item in orderedList)
            {
                ...
                data.Add("<input type='text' class='VoucherNumber form-control hide' value='" + code + "' data-location-Id='" + item.FromLocationId + "' data-location-code='" + item.VoucherCode + "' />" + code + "");
