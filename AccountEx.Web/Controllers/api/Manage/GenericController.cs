...
{
    public class GenericController : GenericApiController<Generic>
    {

        protected override JQueryResponse GetDataTable()
        {
            var queryString = Request.RequestUri.ParseQueryString();
            var coloumns = new[] { "Name", "Description", "" };
            ...
            foreach (var item in orderedList.Skip(displayStart).Take(displayLength))
            {
                ...
                var editIcon = "<i class='fa fa-edit' onclick=\"Generics.Edit(" + item.Id + ")\" title='Edit' ></i>";
                var deleteIcon = "<i class='fa fa-trash-o' onclick=\"Generics.Delete(" + item.Id + ")\" title='Delete' ></i>";
