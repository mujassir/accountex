...
namespace AccountEx.Web.Controllers.api.CRM
{
    public class PMCController : BaseApiController
    {
        ...
        public ApiResponse Post([FromBody]PMC input)
        {

            ApiResponse response;
            try
            {
                input.Date = SiteContext.Current.Fiscal.ToDate;
                var err = PMCManager.ValidateSave(input);
                if (err == "")
                {
                    input.FiscalId = SiteContext.Current.Fiscal.Id;
                    PMCManager.Save(input);
                    response = new ApiResponse
                    {
                        Success = true,
                        Data = input.Id
        ...
        public JQueryResponse GetDataTable()
        {
            try
            {
                ...
                foreach (var item in orderedList.Skip(displayStart).Take(displayLength))
                {
                    ...
                    if (SiteContext.Current.Fiscal.Id == item.FiscalId && ((SiteContext.Current.User.IsAdmin || SiteContext.Current.UserTypeId == CRMUserType.Admin) || (!FiscalSettingManager.IsPmcLocked)))
                    {
                        icons += editIcon;
                        icons += deleteIcon;
                    }
