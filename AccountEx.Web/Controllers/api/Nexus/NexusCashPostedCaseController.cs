...
{
    public class NexusCashPostedCaseController : BaseApiController
    {
        ...
        public ApiResponse Post([FromBody]List<NexusUnpostedCaseSave> unPostedCases)
        {
            ...
            try
            {
                var err = NexusCaseManager.ValidateSave(unPostedCases, NexusCaseType.Cash);
                if (err == "")
                {

                    NexusCaseManager.Save(unPostedCases, NexusCaseType.Cash);
                    response = new ApiResponse
                    {
                        Success = true,
                        Data = null
        ...
        private ApiResponse GetCases()
        {
            ...
            try
            {
                var data = new NexusCaseRepository().GetUnpostedCases(fromDate, toDate, NexusCaseType.Cash);
