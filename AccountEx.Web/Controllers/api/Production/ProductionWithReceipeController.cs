...
{
    public class ProductionWithReceipeController : BaseApiController
    {
        ...
        public ApiResponse Get(int id)
        {
            ...
            try
            {
                ...
                if (key == "nextvouchernumber")
                    voucherNumber = workInProgressRepo.GetNextVoucherNumber(vouchertype);
                var data = workInProgressRepo.GetByVoucherNumber(voucherNumber, key, out next, out previous, vouchertype);
                if (data != null)
                {
        ...
        public ApiResponse Post([FromBody]WorkInProgress input)
        {
            ...
            try
            {
                if (input.VoucherType == VoucherType.None)
                    input.VoucherType = VoucherType.Production;
        ...
        private string ServerValidateSave(WorkInProgress input)
        {
            ...
            try
            {
                ...
                var record = workInProgressRepo.IsVoucherExists(input.VoucherNumber, input.Id, input.VoucherType);
                ...
                record = workInProgressRepo.IsBookNoExists(input.InvoiceNumber, input.Id, input.VoucherType);
                if (record)
                {
