...
{
    public class CarTypeController : GenericApiController<CarType>
    {
        protected override JQueryResponse GetDataTable()
        {
            var queryString = Request.RequestUri.ParseQueryString();
            var coloumns = new[] { "Name", "Unit", "" };
            ...
            foreach (var item in orderedList.Skip(displayStart).Take(displayLength))
            {
                ...
                data.Add(item.Name);
                //data.Add(item.Unit);
                var editIcon = "<i class='fa fa-edit' onclick=\"CarTypes.Edit(" + item.Id + ")\" title='Edit' ></i>";
                var deleteIcon = "<i class='fa fa-trash-o' onclick=\"CarTypes.Delete(" + item.Id + ")\" title='Delete' ></i>";
