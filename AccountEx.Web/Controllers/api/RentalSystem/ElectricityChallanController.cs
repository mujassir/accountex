...
{
    public class ElectricityChallanController : BaseApiController
    {

        //This method is user to print Rent Challan(Call is genrated from Rent Challan Page)
        public ApiResponse Get(int month, int year, int accountid, int shopId, int id, bool loadAllPrevious)
        {
            ApiResponse response;
            try
            {
                var data = new RentMonthlyLiabilityRepository().GetElectrictyByMonthYearTenant(month, year, accountid, id, loadAllPrevious);
        ...
        public ApiResponse Get(int Id, bool printChallan)
        {
            ...
            try
            {
                var data = new RentMonthlyLiabilityRepository().PrintElectricityChallan(Id);
                response = new ApiResponse
                {
                    Success = true,
                    Data = new
        ...
        public ApiResponse Post([FromBody]Challan input, bool createChallan)
        {
            ...
            try
            {
                ...
                var toDate = new DateTime(input.Year, input.Month, days);
                input.FromDate = fromDate;
                input.ToDate = toDate;
                input.ToMonth = input.Month;
                input.ToYear = input.Year;
                ...
                if (err == "")
                {
                    ...
                    ChallanManager.CreateElectricityChallan(input);
                    response = new ApiResponse
                    {
                        Success = true,
                        Data = input
        ...
        public ApiResponse Delete(int id)
        {
            ...
            try
            {
                ...
                if (err == "")
                {
                    RentMonthlyLiabilityManager.DeleteElectricity(id);
        ...
        public static string ValidateCreateChallan(Challan input)
        {
            ...
            try
            {
                ...
                var netBilling = rd.ElectricityCharges + rd.ElectricityArrears + rd.SurCharge;
                if (alreadyChallanTotal + currentTotal > netBilling)
                {
                    err += "total challan amount can't exceed net billing.,";
                }
                var lpChallan = new ChallanRepository().GetLastPeriodChallan(input.FromDate.Value, Numerics.GetInt(input.RentAgreementId), VoucherType.ElectictyChallan);
                if (lpChallan != null)
                {
                    data = new RentMonthlyLiabilityRepository().GetElectrictyByMonthYearTenant(lpChallan.Month, lpChallan.Year, Numerics.GetInt(lpChallan.TenantAccountId), Numerics.GetInt(lpChallan.RentAgreementId));

                    alreadyChallanTotal = data.AllChallans.Sum(p => p.TotalAmount);
                    rd = data.ElectricityChallan;
                    netBilling = rd.ElectricityCharges + rd.ElectricityArrears + rd.SurCharge;
                    ...
                    if (lpChallan.ToDate.Value.AddDays(1) != input.FromDate)
                    {
                        ...
                        err += "challan is missing for the month of " + month + ".please create previous month challan.,";
                    }
                }
